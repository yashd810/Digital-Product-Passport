...
import { formatPassportStatus, normalizePassportStatus } from "./passportStatus";
...
function isPublishedHistoryStatus(status) {
...
function ArchivedPassports({ user, companyId }) {
  ...
  const getArchivedPublicVersionNumber = useCallback((passport) => {
    if (
      isPublishedHistoryStatus(passport?.release_status) &&
  ...
  const bulkExportJson = async () => {
    ...
    try {
      ...
      a.download = `archived-passports-${new Date().toISOString().slice(0, 10)}.json`;
      a.click();
      URL.revokeObjectURL(a.href);
      showSuccess(`Exported ${exported.length} archived passport${exported.length !== 1 ? "s" : ""}`);
    } catch (e) { showError(e.message); }
    finally { setBulkLoading(false); }
  };
