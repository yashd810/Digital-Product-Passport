...
export function usePassportListActions({
  ...
  const downloadQrCodes = useCallback(async ({ widthMm, heightMm, format, colorMode }) => {
    ...
    const isBlackAndWhite = colorMode === "bw";
  ...
  const bulkExportJson = useCallback(async () => {
    ...
    try {
      ...
      link.download = `passports-export-${new Date().toISOString().slice(0, 10)}.json`;
      link.click();
