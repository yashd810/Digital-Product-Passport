        ...
        if (text.includes("backup_provider_id, backup_provider_key") && text.includes("payload_hash")) {
          return {
            rows: [{
              ...
              passportType: "battery",
              versionNumber: 2,
              dppId: "did:web:www.example.test:did:dpp:item:dpp_auto_1",
        ...
        if (text.includes("INSERT INTO backup_public_handovers")) {
          return {
            rows: [{
              ...
              lineageId: params[2],
              passportType: params[3],
              internalAliasId: params[4],
              versionNumber: params[5],
  ...
  test("replicates a passport snapshot through the configured storage layer", async () => {
    ...
    const result = await service.replicatePassportSnapshot({
      passport: {
        ...
        passportType: "battery",
        versionNumber: 2,
        uniqueProductIdentifier: "did:web:www.example.test:did:battery:item:c5-bat-2026-001",
      },
      typeDef: { type_name: "battery" },
      companyName: "Acme Energy",
      reason: "release",
