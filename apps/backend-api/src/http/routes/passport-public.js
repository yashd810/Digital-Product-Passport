...
module.exports = function registerPassportPublicRoutes(app, {
  ...
  function securityGroupReadLimiter(req, res, next) {
    ...
    // A security-group key can add restricted fields to an otherwise public
    // response.  Its value is carried in a request header, which is not part
    // of a normal HTTP cache key, so never let that representation be stored
    // and reused by an anonymous viewer of the same passport URL.
