...
/**
 * Process-local cache provider factory.
 *
 * This platform adapter keeps cache-provider selection outside feature modules.
 * Features depend only on the returned `get`/`set`/`del`/`wrap` contract, so a
 * distributed provider can replace the in-memory implementation later.
 */
