"use strict";
const registerAnalyticsRoutes = require("../src/modules/admin/register-analytics-routes");
...
const registerSuperAdminRoutes = require("../src/modules/admin/register-super-admin-routes");
...
const RESERVED_PASSPORT_FIELD_TOKEN_MAP = new Map(
  RESERVED_PASSPORT_FIELD_KEYS.map((key) => [normalizeReservedToken(key), key])
);

const RESERVED_PASSPORT_SEMANTIC_TOKEN_MAP = new Map(
  ...
  function findReservedPassportHeaderFieldConflicts(sections = []) {
    ...
    for (const section of sections || []) {
      for (const field of section?.fields || []) {
        const reservedKey = RESERVED_PASSPORT_FIELD_TOKEN_MAP.get(normalizeReservedToken(field?.key));
        if (reservedKey) {
          conflicts.push({
            field: field.key,
            conflictType: "key",
            reservedField: reservedKey,
            message: `Field "${field.key}" is already generated by the passport registry/header as "${reservedKey}" and does not need to be created again.`
          });
        }

        const semanticId = field?.semanticId || field?.semantic_id || null;
        const reservedSemanticId = RESERVED_PASSPORT_SEMANTIC_TOKEN_MAP.get(normalizeReservedToken(semanticId));
        if (reservedSemanticId) {
          conflicts.push({
            field: field.key,
            semanticId,
            conflictType: "semanticId",
            reservedField: reservedSemanticId,
            message: `Field "${field.key}" uses reserved semanticId "${reservedSemanticId}", which is already generated by the passport registry/header and does not need to be created again.`
          });
        }
      }
    }
