...
export const backendApiFamilies = [
  ...
  },
  {
    ...
    details: [
      ...
      "Keeps reusable content company-scoped. Template list, detail, create, update, delete, and draft export are also limited to passport types with an active company grant.",
    ],
  },
  {
...
export const companyWriteApiTable = {
  ...
  rows: [
    ["Create one passport", "POST /api/companies/:companyId/passports", "Session cookie or bearer token, company access, an active grant for passportType, editor or company admin", "{ passportType, modelName, internalAliasId, granularity, uniqueProductIdentifier, economicOperatorId, facilityId, ...fieldKeys }", "Creates one new draft passport. internalAliasId must be unique."],
    ["Bulk create many passports", "POST /api/companies/:companyId/passports/bulk", "Session cookie or bearer token, company access, an active grant for passportType, editor or company admin", "{ passportType, passports: [ {...}, {...} ] } up to 500 rows", "Creates many passports and returns a per-row summary instead of failing the whole batch."],
    ["Update one editable passport", "PATCH /api/companies/:companyId/passports/:dppId", "Session cookie or bearer token, company access, an active grant for passportType, editor or company admin", "{ passportType, granularity, uniqueProductIdentifier, economicOperatorId, facilityId, ...fieldsToChange }", "Updates one draft or in-revision passport. Released granularity cannot be changed in place."],
    ["Bulk update matched passports", "PATCH /api/companies/:companyId/passports", "Session cookie or bearer token, company access, an active grant for passportType, editor or company admin", "{ passportType, passports: [ { dppId or internalAliasId, ...fields }, ... ] } up to 500 rows", "Updates many existing editable passports. It does not create new ones."],
    ["Bulk update many records with the same value", "PATCH /api/companies/:companyId/passports/bulk-update-all", "Session cookie or bearer token, company access, an active grant for passportType, editor or company admin", "{ passportType, filter, update }", "Applies one update object to every matching editable passport. internalAliasId cannot be bulk-set."],
    ["Upsert from CSV text", "POST /api/companies/:companyId/passports/upsert-csv", "Session cookie or bearer token, company access, an active grant for passportType, editor or company admin", "{ passportType, csv: \"...csv text...\" } up to 500 data rows", "Creates new passports when no dppId is present, or updates matching editable passports when dppId or internalAliasId matches. passportType is the request envelope, not an imported field row."],
    ["Upsert from JSON", "POST /api/companies/:companyId/passports/upsert-json", "Session cookie or bearer token, company access, an active grant for passportType, editor or company admin", "{ passportType, passports: [ {...}, {...} ] } or a raw array, up to 500 rows", "Creates new passports without dppId, or updates editable ones when dppId or internalAliasId matches. passportType is the request envelope, not an imported passport field."],
    ["Release one passport", "PATCH /api/companies/:companyId/passports/:dppId/release", "Session cookie or bearer token, company access, an active grant for passportType, editor or company admin", "{ passportType }", "Moves an editable passport to released and stores signature/VC metadata."],
    ["Revise one released passport", "POST /api/companies/:companyId/passports/:dppId/revise", "Session cookie or bearer token, company access, an active grant for passportType, editor or company admin", "{ passportType }", "Creates the next editable version from the latest released version."],
    ["Change granularity with a linked successor", "POST /api/companies/:companyId/passports/:dppId/granularity-transition", "Session cookie or bearer token, company access, an active grant for passportType, editor or company admin", "{ passportType, targetGranularity, reason }", "Creates a linked successor identifier when released DPP granularity must move between model, batch, and item levels."],
    ["Bulk revise passports", "POST /api/companies/:companyId/passports/bulk-revise", "Session cookie or bearer token, company access, an active grant for each requested passport type, editor or company admin", "{ items, changes, submitToWorkflow, reviewerId, approverId, ... }", "Creates revised copies for many released passports and can optionally move them toward workflow."],
    ["Submit into workflow", "POST /api/companies/:companyId/passports/:dppId/submit-review", "Session cookie or bearer token, company access, an active grant for passportType, editor or company admin", "{ passportType, reviewerId, approverId }", "Places the passport into reviewer and or approver workflow."],
    ["Bulk release passports", "POST /api/companies/:companyId/passports/bulk-release", "Session cookie or bearer token, company access, an active grant for each requested passport type, editor or company admin", "{ items: [ { dppId, passportType } ] } up to 500", "Releases many draft or in-revision passports at once, signing each one. Skips already-released rows."],
    ["Bulk submit to workflow", "POST /api/companies/:companyId/passports/bulk-workflow", "Session cookie or bearer token, company access, an active grant for each requested passport type, editor or company admin", "{ items: [ { dppId, passportType } ], reviewerId, approverId }", "Submits many editable passports into the review and approval workflow in one request."],
    ["Archive one passport", "POST /api/companies/:companyId/passports/:dppId/archive", "Session cookie or bearer token, company access, an active grant for passportType, editor or company admin", "{ passportType }", "Copies all versions to the passportArchives table, then soft-deletes from the passport table. The passport disappears from the active list and analytics."],
    ["Bulk archive passports", "POST /api/companies/:companyId/passports/bulk-archive", "Session cookie or bearer token, company access, an active grant for each requested passport type, editor or company admin", "{ items: [ { dppId, passportType } ] } up to 500", "Archives many passports at once and reports how many were archived or skipped."],
    ["Unarchive one passport", "POST /api/companies/:companyId/passports/:dppId/unarchive", "Session cookie or bearer token, company access, and an active grant for the archived record's passport type, editor or company admin", "No body", "Restores all soft-deleted versions and removes the archive entries. The passport reappears in the active list."],
    ["Bulk unarchive passports", "POST /api/companies/:companyId/passports/bulk-unarchive", "Session cookie or bearer token, company access, and an active grant for each archived record's passport type, editor or company admin", "{ dppIds: [ \"uuid\", ... ] } up to 500", "Restores many archived passports and reports how many were restored or skipped."],
    ["Delete one editable passport", "DELETE /api/companies/:companyId/passports/:dppId", "Session cookie or bearer token, company access, an active grant for passportType, editor or company admin", "{ passportType }", "Permanently hard-deletes a draft, including its registry/dependent records. Other editable statuses such as inRevision are archived as a pre-delete snapshot and soft-deleted. Released passports cannot be deleted."],
    ["Bulk delete editable passports", "DELETE /api/companies/:companyId/passports", "Session cookie or bearer token, company access, an active grant for passportType, editor or company admin", "{ passportType, identifiers: [ { dppId }, { internalAliasId } ] } up to 500", "Applies the same rule per match: drafts are hard-deleted, while other editable statuses are snapshotted and soft-deleted; released rows are skipped. Returns deleted, skipped, and failed details."],
  ],
};
...
export const readExportApiTable = {
  ...
  rows: [
    ["List company passports", "GET /api/companies/:companyId/passports", "Session cookie or bearer token, company access, and an active grant for passportType", "Query params: passportType required, search optional, status optional", "Current active company passports for that granted type. Archived passports are excluded."],
    ["List archived passports", "GET /api/companies/:companyId/passports/archived", "Session cookie or bearer token and company access; results are limited to actively granted types", "Query params: passportType optional, search optional", "Returns the latest version per DPP ID from the passportArchives table only for types the company can still use, with archived-by user details."],
    ["Fetch many by dppId or internalAliasId", "POST /api/companies/:companyId/passports/bulk-fetch", "Session cookie or bearer token, company access, and an active grant for passportType", "{ passportType, identifiers: [ { dppId }, { internalAliasId } ] }", "A found or notFound result for each requested identifier."],
    ["Export drafts or released rows", "GET /api/companies/:companyId/passports/export-drafts", "Session cookie or bearer token, company access, and an active grant for passportType", "Query params: passportType required, format csv or json, status draft released inRevision or all", "A downloadable CSV or JSON-LD export for that granted type."],
    ["Fetch one company passport", "GET /api/companies/:companyId/passports/:dppId", "Session cookie or bearer token, company access, and an active grant for passportType", "Query param: passportType required; versionNumber and representation optional", "The requested company-visible version of that passport."],
    ["Preview a company passport", "GET /api/companies/:companyId/passports/:passportKey/preview", "Session cookie or bearer token, company access, and an active grant for the record's passport type", "No body", "Authenticated preview payload before public release. The dashboard renders it on protected /dpp/preview/...; it is not a public share URL."],
    ["Check compliance status", "GET /api/companies/:companyId/passports/:dppId/compliance", "Session cookie or bearer token, company access, and an active grant for passportType", "Query param: passportType required", "Compliance summary for the current passport."],
    ["See version diff input", "GET /api/companies/:companyId/passports/:dppId/diff", "Session cookie or bearer token, company access, and an active grant for passportType", "Query param: passportType", "All versions needed for compare views."],
    ["See passport history", "GET /api/companies/:companyId/passports/:dppId/history", "Session cookie or bearer token, company access, and an active grant for the record's passport type", "No body", "Version history including non-public data for authorized company users."],
    ["See identifier lineage", "GET /api/companies/:companyId/passports/:dppId/identifier-lineage", "Session cookie or bearer token, company access, and an active grant for the record's passport type", "No body", "Lineage for DID/product identifier transitions."],
    ...
    ["Get current edit lock", "GET /api/companies/:companyId/passports/:dppId/edit-session", "Session cookie or bearer token, company access, and an active grant for the record's passport type", "No body", "Shows whether another user is actively editing."],
    ["Start or refresh edit lock", "POST /api/companies/:companyId/passports/:dppId/edit-session", "Session cookie or bearer token, company access, an active grant for the record's passport type, editor or company admin", "No body", "Marks the current user as the active editor."],
    ["Clear edit lock", "DELETE /api/companies/:companyId/passports/:dppId/edit-session", "Session cookie or bearer token, company access, and an active grant for the record's passport type", "No body", "Ends the current edit session."],
  ],
};
