export const coreDatabaseTables = [
  {
    ...
    tables: [
      ...
      },
      {
        ...
        columns: ["id", "token", "email", "companyId", "invitedBy", "roleToAssign", "used", "expiresAt", "createdAt"],
      },
      {
        name: "passwordResetTokens",
        purpose: "Password recovery tokens and expiry tracking.",
        columns: ["id", "userId", "token", "used", "expiresAt", "createdAt"],
      },
      {
        name: "apiKeys",
        purpose: "Company-level read API keys used with `X-API-Key` on scoped `/api/v1/passports*` endpoints.",
        columns: ["id", "companyId", "name", "keyHash", "keyPrefix", "keySalt", "hashAlgorithm", "scopes", "expiresAt", "createdBy", "createdAt", "lastUsedAt", "isActive"],
      },
    ],
  },
  {
    ...
    tables: [
      {
        ...
        columns: ["id", "userId", "companyId", "audience", "grantedBy", "reason", "expiresAt", "isActive", "createdAt", "updatedAt"],
      },
      {
        name: "passportAccessGrants",
        purpose: "Passport-level delegated access grants for an audience, user, and optional element path.",
        columns: ["id", "passportDppId", "companyId", "audience", "elementIdPath", "granteeUserId", "grantedBy", "reason", "expiresAt", "isActive", "createdAt", "updatedAt"],
      },
      {
...
export const backendOperationFlows = [
  ...
  },
  {
    ...
    steps: [
      "A QR code or copied link opens the public `/p/:internalAliasId` route, which then loads the correct consumer-facing viewer for that passport type.",
...
export const publicAndLiveApiTable = {
  ...
  rows: [
    ...
    ["Read by product identifier", "GET /api/v1/dppsByProductId/:internalAliasId", "No auth for public released data", "Optional companyId when ambiguous", "Resolves released DPP data by local product ID or product DID."],
    ["Read many by identifiers", "POST /api/v1/dppsByProductIds or /search", "No auth for public released data", "{ internalAliasId: [...] } plus optional cursor, limit, and filters", "Bulk lookup for released DPPs by product identifiers."],
    ["Read version or point-in-time", "GET /api/v1/dpps/:productIdentifier/versions/:versionNumber or /api/v1/dppsByProductIdAndDate/:internalAliasId", "No auth for public released data", "Version number or date query", "Returns a specific released version or the released version valid at a date."],
    ...
    ["Public passport view", "GET /api/passports/by-product/:internalAliasId", "No auth", "Optional query param: version", "The latest released public-safe passport view for a product, with restricted fields removed."],
    ...
    ["Public passport history", "GET /api/passports/by-product/:internalAliasId/history", "No auth", "No body", "Public version history only."],
