...
function BulkModal({ passportType, typeLabel, companyId, templateId, templateName, onClose, onDone }) {
  ...
  const handleCreate = async () => {
    ...
    try {
      let prefill = {};
      if (templateId) {
        const tr = await fetch(`${API}/api/companies/${companyId}/templates/${templateId}`, { headers: authHeaders() });
        if (tr.ok) {
...
export default function CreateHub({ user, companyId }) {
  ...
  const loadTemplates = useCallback((type) => {
    ...
    fetch(`${API}/api/companies/${companyId}/templates?passport_type=${type.type_name}`, { headers: authHeaders() })
      .then(r => r.ok ? r.json() : [])
      .then(setTemplates)
      .catch(() => setTemplates([]))
