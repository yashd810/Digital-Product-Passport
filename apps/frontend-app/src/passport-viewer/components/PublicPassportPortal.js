...
function filterViewerSections(sections = []) {
  return normalizeSchemaSections(sections)
    .map((section) => ({
      ...section,
      fields: (section.fields || []).filter((field) => !isViewerHiddenField(field)),
      sections: filterViewerSections(section.sections || []),
    }))
...
function getCompositionItems(field, raw) {
  ...
  if (!["table", "objectList"].includes(field.type)) return null;
              ...
              {sections.map((section, sectionIndex) => {
                ...
                return (
                  <button
                    ...
                    onClick={() => setActiveDataSectionKey(section.key || translateSchemaLabel(lang, section))}
                  >
                    <span>{translateSchemaLabel(lang, section)}</span>
                  </button>
                );
              })}
            </aside>

            <div className="data-section-panel">
              {selectedDataSection ? (
                <>
                  <div className="data-section-panel-head">
                    <h3>{translateSchemaLabel(lang, selectedDataSection)}</h3>
                  </div>

                  {(selectedDataSection.fields || []).length > 0 && (
                    <div className="data-direct-field-card">
                      <DataFieldRows
                        fields={selectedDataSection.fields || []}
                        lang={lang}
                        {...dataFieldProps}
                      />
                    </div>
                ...
                <div className="data-empty-state">No passport data sections are available.</div>
              )}
            </div>
          </div>
        </section>
