...
export function SectionView({
  ...
  const fieldEntries = visibleFields.map(f => {
    ...
    } else if (f.rangeKind && semanticGraph) {
      display = (
        <SemanticGraphValue
          graph={semanticGraph}
          property={getRootSemanticProperty(semanticGraph, f.key) || f}
          value={raw}
    ...
    let pieItems = null;
    if (f.composition && ["table", "objectList"].includes(f.type) && raw) {
...
// ─── Print view ───────────────────────────────────────────────
export function PrintView({ passport, companyData, sections, semanticGraph = null }) {
  const isFileUrl = v => typeof v === "string" && v.startsWith("http");
  const statusLabel = formatPassportStatus(passport.releaseStatus);
  const renderFieldRows = (section) => {
  ...
  const renderPrintSection = (section, depth = 0, path = []) => {
    const children = section.sections || section.groups || [];
    const sectionKey = [...path, section.key || section.label || depth].join(".");
    return (
      <div key={sectionKey} className={`print-section print-section-depth-${Math.min(depth, 3)}`}>
        {depth === 0
          ? <h2 className="print-section-title">{section.label}</h2>
          : <h3 className="print-section-subtitle">{section.label}</h3>}
        {renderFieldRows(section)}
        {children.map((child) => renderPrintSection(child, depth + 1, [...path, section.key || section.label || depth]))}
      </div>
    );
  };
  return (
    <div className="print-view">
      ...
      {sections.map((section) => renderPrintSection(section))}
