/**
 * Browser workspace controller for the Passport Module Generator.
 *
 * Coordinates the editable module, fields, semantic graph, preview, and
 * browser storage. Cross-environment CSV and schema rules are loaded from
 * `../shared/` before this file runs.
 */
"use strict";


// ─── BROWSER DEPENDENCY CONTRACTS ────────────────────────────────────────────
// The HTML file loads these focused modules first. Fail fast if that contract
// changes, rather than letting a half-initialized editor lose user input.
...
const smoothDetails = globalThis.PassportModuleSmoothDetails;
if (!smoothDetails) {
  throw new Error("The smooth details controller did not load.");
}
const { setupSmoothDetails } = smoothDetails;
const searchableSelects = globalThis.PassportModuleSearchableSelects;
if (!searchableSelects) {
  throw new Error("The searchable select controller did not load.");
}
const {
...
const headerMapping = globalThis.PassportModuleHeaderMapping;
if (!headerMapping) {
  throw new Error("The passport header mapping helper did not load.");
}
const {
...
// content separate from browser-state and editor orchestration.
const sample = globalThis.PassportModuleWorkspaceSample;
if (!sample) {
  throw new Error("The workspace starter specification did not load.");
}
...
// ─── UI EVENT WIRING AND INITIALIZATION ─────────────────────────────────────
// Bind controls only after every helper above exists, then restore a safe local
// session. No handler writes to the backend: this tool remains export-only.
