...
const createAasSigningService  = require("../services/aas-signing");
...
const ASSET_MANAGEMENT_DIR = path.join(APP_ROOT_DIR, "Asset Management");
const LOCAL_STORAGE_DIR = path.resolve(
  process.env.LOCAL_STORAGE_DIR || path.join(APP_ROOT_DIR, "Local Storage")
);
...
// ─── AAS SIGNING SERVICE ─────────────────────────────────────────────────────
const aasSigning = createAasSigningService({ pool, crypto, canonicalize });
const { buildAasExport, signPassport, verifyPassportSignature } = aasSigning;
...
pool.query("SELECT NOW()")
  .then(async () => {
    ...
    await aasSigning.loadOrGenerateSigningKey();
...
registerPassportRoutes(app, {
  ...
  submitPassportToWorkflow, signPassport, buildAasExport,
});
...
registerPassportPublicRoutes(app, {
  ...
  buildAasExport, verifyPassportSignature, aasSigning,
});
