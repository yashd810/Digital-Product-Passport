...
module.exports = function registerPassportPublicRoutes(app, {
  ...
  // ─── AAS EXPORT (public, released only) ────────────────────────────────
  ...
  app.get("/.well-known/did.json", async (_req, res) => {
    try {
      if (!aasSigning.getSigningKey()) return res.status(503).json({ error: "Signing key not loaded" });
      const appUrl = process.env.APP_URL || "http://localhost:3001";
      const domain = new URL(appUrl).host;
      const did    = `did:web:${domain}`;

      const pubKey = crypto.createPublicKey(aasSigning.getSigningKey().publicKey);
      ...
      const didDocument = {
        ...
        verificationMethod: [{
          ...
          type:         "JsonWebKey2020",
          controller:   did,
          publicKeyJwk: { ...jwk, kid: aasSigning.getSigningKey().keyId },
        }],
