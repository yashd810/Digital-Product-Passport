...
module.exports = function registerPassportRoutes(app, {
  ...
  // ─── AAS EXPORT (authenticated) ───────────────────────────────────────────

  app.get("/api/companies/:companyId/passports/:guid/export/aas", authenticateToken, checkCompanyAccess, async (req, res) => {
    try {
      const { companyId, guid } = req.params;
      const reg = await pool.query(
        "SELECT passport_type FROM passport_registry WHERE guid = $1 AND company_id = $2",
      ...
      let r = await pool.query(
        `SELECT * FROM ${tbl} WHERE guid = $1 AND deleted_at IS NULL ORDER BY version_number DESC LIMIT 1`, [guid]
      );
      if (!r.rows.length) {
      ...
      const dynamicValues = Object.fromEntries(dynRes.rows.map(r => [r.field_key, { value: r.value, updatedAt: r.updated_at }]));
