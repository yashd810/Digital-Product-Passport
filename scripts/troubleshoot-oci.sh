...
echo "   - Do not run /opt/dpp/infra/oracle/deploy-prod.sh from the console session."
echo "   - Return to the trusted deployment runner and invoke:"
echo "     DPP_DEPLOY_TARGET=<frontend|backend|all> bash scripts/deploy/deploy-to-oci.sh"
echo "   - The root-owned release entry point validates and stages the exact committed revision before Docker runs."
echo ""
